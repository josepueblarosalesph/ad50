@foreach ($experiencias as $index => $experiencia)
    <fieldset class="rounded-[14px] border border-line-2 p-5" wire:key="experiencia-{{ $index }}">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                <x-combobox model="experiencias.{{ $index }}.cargo" label="Cargo u ocupación *" catalogo="cargo" :valor="$experiencia['cargo'] ?? ''" error="experiencias.{{ $index }}.cargo" placeholder="Escribe para buscar" fijo="Otros" fijo-ayuda="Mi cargo no está en la lista" />
            ...
            <div>
                <x-combobox model="experiencias.{{ $index }}.empresa" label="Empresa *" catalogo="empresa" :valor="$experiencia['empresa'] ?? ''" error="experiencias.{{ $index }}.empresa" placeholder="Escribe para buscar" fijo="Otra" fijo-ayuda="Mi empresa no está en la lista" />
