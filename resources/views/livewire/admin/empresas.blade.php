<div class="ad-panel">
    ...
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <span class="ad-chip ad-chip-orange ad-chip-dot">{{ $pendientes->total() }} pendientes</span>
    </div>
    ...
    <section class="space-y-4">
        <div class="flex items-center justify-between"><h2 class="text-[18px] font-extrabold">Pendientes de revisión</h2><span class="text-[13px] font-semibold text-gray-500">{{ $pendientes->total() }} solicitudes</span></div>
        @forelse ($pendientes as $empresa)
            {{-- Datos personales: fuera de las grabaciones de Clarity (ver partials/clarity). --}}
            <article class="ad-card p-6" data-clarity-mask="true" wire:key="empresa-pendiente-{{ $empresa->id }}">
        ...
        @if ($pendientes->hasPages())
            <div class="mt-6">{{ $pendientes->links() }}</div>
        @endif
    </section>

    <div class="mt-8 grid gap-5 lg:grid-cols-2">
        <section class="ad-card overflow-hidden"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Aún sin antecedentes</h2><span class="ad-chip">{{ $inactivas->total() }}</span></div><div class="divide-y divide-line" data-clarity-mask="true">@forelse ($inactivas as $empresa)<div wire:key="inactiva-{{ $empresa->id }}" class="p-4"><b class="text-[14px]">{{ $empresa->razon_social }}</b><p class="mt-1 text-[12px] text-gray-500">{{ $empresa->user->email }}</p>@if ($empresa->user->email_verified_at === null){{-- Sin verificar no puede ni llegar a enviar antecedentes: el middleware `verified` la deja fuera. Se queda aquí para siempre si nadie la rescata. --}}<span class="ad-chip ad-chip-sm ad-chip-orange mt-2">Correo sin verificar</span><div class="mt-2 flex flex-wrap gap-2"><x-acciones-verificacion :user="$empresa->user" /></div>@endif<div class="mt-2"><x-plan-empresa-admin :empresa="$empresa" /></div></div>@empty<p class="p-6 text-[13px] text-gray-500">Todas las empresas enviaron sus datos.</p>@endforelse</div>@if ($inactivas->hasPages())<div class="border-t border-line p-4">{{ $inactivas->links() }}</div>@endif</section>
        <section class="ad-card overflow-hidden"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Empresas activas</h2><span class="ad-chip ad-chip-green">{{ $activas->total() }}</span></div><div class="divide-y divide-line" data-clarity-mask="true">@forelse ($activas as $empresa)<div wire:key="activa-{{ $empresa->id }}" class="p-4"><b class="text-[14px]">{{ $empresa->razon_social }}</b><p class="mt-1 text-[12px] text-gray-500">Habilitada {{ $empresa->activada_at?->translatedFormat('d M Y') }}{{ $empresa->activadaPor ? ' por '.$empresa->activadaPor->name : '' }}</p><div class="mt-2"><x-plan-empresa-admin :empresa="$empresa" /></div></div>@empty<p class="p-6 text-[13px] text-gray-500">Todavía no hay empresas activas.</p>@endforelse</div>@if ($activas->hasPages())<div class="border-t border-line p-4">{{ $activas->links() }}</div>@endif</section>
    </div>
