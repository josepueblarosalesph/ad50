<div>
    ...
    <x-slot:nav>
        ...
        <a href="#empresas" class="text-[13.5px] font-semibold px-3.5 py-2 rounded-lg text-gray-500 hover:text-ink">Empresas</a>
    ...
    <x-slot:sidebar>
        ...
        @foreach ([['squares-2x2','Resumen'], ['building-office-2','Empresas'], ['user','Postulantes'], ['bars-3','Catálogos'], ['credit-card','Suscripciones'], ['shield-check','Seguridad y auditoría']] as [$icon, $label])
            <a href="#" @class(['flex items-center gap-3 text-[14px] font-semibold px-3 py-2.5 rounded-[10px]', 'bg-orange-100 text-orange-600' => $loop->first, 'text-gray-700 hover:bg-paper' => !$loop->first])><flux:icon :name="$icon" class="size-[18px]" />{{ $label }}</a>
        @endforeach
    </x-slot:sidebar>
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        @foreach ([['Empresas activas', $totalEmpresas, 'Organizaciones registradas'], ['Postulantes', $totalPostulantes, 'Perfiles profesionales'], ['Búsquedas activas', $totalBusquedas, 'Procesos en curso'], ['Coincidencias generadas', $totalCoincidencias, 'Matches acumulados']] as [$label, $value, $detail])
    ...
    <div class="grid xl:grid-cols-[1.4fr_0.8fr] gap-5 items-start">
        <section id="empresas" class="ad-card overflow-hidden"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Empresas recientes</h2><span class="text-[12.5px] font-semibold text-orange-600">Ver todas</span></div><div class="overflow-x-auto"><table class="w-full text-[13.5px]"><thead><tr class="text-left text-[11px] uppercase tracking-wider text-gray-400 border-b border-line"><th class="p-4">Empresa</th><th class="p-4">Plan</th><th class="p-4">Estado</th></tr></thead><tbody>
            @forelse ($empresas as $empresa)
                <tr class="border-b border-line last:border-0"><td class="p-4"><b>{{ $empresa->razon_social }}</b><p class="text-[11.5px] text-gray-500 mt-1">{{ $empresa->user->email }}</p></td><td class="p-4 text-gray-500">{{ $empresa->plan?->nombre ?? 'Sin plan' }}</td><td class="p-4"><span class="ad-chip ad-chip-green">Activa</span></td></tr>
            @empty
