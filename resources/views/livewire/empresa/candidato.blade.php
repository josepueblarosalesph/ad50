<div>
    ...
    <div class="flex items-center justify-between gap-5 mb-6 flex-wrap"><div class="flex items-center gap-4"><div class="size-16 rounded-full bg-orange-100 text-orange-600 grid place-items-center" aria-hidden="true"><flux:icon.user class="size-7" /></div><div><h1 class="text-[24px] font-extrabold">{{ $postulante->user->name }}</h1><p class="text-[14px] text-gray-500 mt-1">{{ $postulante->titular ?: ($postulante->carrera ?: 'Titular profesional no informado') }}</p><span @class(['ad-chip mt-2', 'ad-chip-green' => $match->estado_match === 'cumple'])>{{ $match->estado_match === 'cumple' ? 'Cumple' : 'Parcial — cumple' }} {{ $match->criterios_cumplidos }} de {{ $match->criterios_totales }} criterios</span></div></div><button type="button" wire:click="toggleFavorito" wire:loading.attr="disabled" @class(['ad-favorite-button ad-btn-sm inline-flex items-center gap-2 rounded-xl border font-bold transition disabled:opacity-50', 'is-active' => $match->favorito]) aria-pressed="{{ $match->favorito ? 'true' : 'false' }}"><flux:icon.star variant="solid" class="size-5" />{{ $match->favorito ? 'Guardado en favoritos' : 'Guardar como favorito' }}</button></div>
    ...
    <div class="grid lg:grid-cols-[1.4fr_0.8fr] gap-5 items-start">
        <div class="space-y-5">
            ...
            @endif
            <section class="ad-card"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Intereses</h2></div><div class="space-y-5 p-6"><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Regiones de interés</h3><div class="flex flex-wrap gap-2">@forelse ($postulante->regiones_interes ?? [] as $region)<span class="ad-candidate-value"><flux:icon.map-pin />{{ $region }}</span>@empty<span class="text-[13px] text-gray-500">Sin regiones informadas</span>@endforelse</div></div><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Industrias de interés</h3><div class="flex flex-wrap gap-2">@foreach ($postulante->industrias_interes ?? [] as $industria)<span class="ad-candidate-value"><flux:icon.building-office-2 />{{ $industria }}</span>@endforeach</div></div><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Modalidad de trabajo</h3><div class="flex flex-wrap gap-2">@forelse ($postulante->modalidad_trabajo ?? [] as $modalidad)<span class="ad-candidate-value"><flux:icon.clock />{{ $modalidad }}</span>@empty<span class="text-[13px] text-gray-500">Sin modalidad informada</span>@endforelse</div></div></div></section>
            @if (filled($postulante->habilidades))<section class="ad-card"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Habilidades</h2></div><div class="p-6"><div class="flex flex-wrap gap-2">@foreach ($postulante->habilidades as $habilidad)<span class="ad-candidate-value">{{ $habilidad }}</span>@endforeach</div></div></section>@endif
