<div>
    ...
    <div class="grid lg:grid-cols-[1.4fr_0.8fr] gap-5 items-start">
        <div class="space-y-5">
            ...
            @endif
            <section class="ad-card"><div class="ad-card-head"><h2 class="text-[16px] font-bold">Intereses</h2></div><div class="space-y-5 p-6"><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Regiones de interés</h3><div class="flex flex-wrap gap-2">@forelse (collect([$postulante->region_interes, $postulante->region_interes_2, $postulante->region_interes_3])->filter() as $region)<span class="ad-candidate-value"><flux:icon.map-pin />{{ $region }}</span>@empty<span class="text-[13px] text-gray-500">Sin regiones informadas</span>@endforelse</div></div><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Industrias de interés</h3><div class="flex flex-wrap gap-2">@foreach (collect([$postulante->industria, $postulante->industria_2, $postulante->industria_3])->filter() as $industria)<span class="ad-candidate-value"><flux:icon.building-office-2 />{{ $industria }}</span>@endforeach</div></div><div><h3 class="mb-2 text-[12px] font-extrabold uppercase tracking-wide text-gray-400">Modalidad de trabajo</h3>@if ($postulante->modalidad_trabajo)<span class="ad-candidate-value"><flux:icon.clock />{{ $postulante->modalidad_trabajo }}</span>@else<span class="text-[13px] text-gray-500">Sin modalidad informada</span>@endif</div></div></section>
